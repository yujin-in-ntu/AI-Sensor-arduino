/*
  카메라 5단계: 종이에 쓴 숫자를 촬영해 보드 안에서 추론합니다.
  시리얼 모니터에서 p를 보내면 즉시 한 장을 촬영합니다.
*/
...
bool prepareInput() {
  ...
  for (int i = 0; i < IMAGE_SIZE * IMAGE_SIZE; ++i) {
    float normalized = digitImage[i] / 255.0f;
    int value = (int)roundf(normalized / scale) + zeroPoint;
...
void sendPrediction(bool guiMode) {
  ...
  for (unsigned int i = 0; i < g_class_count; ++i) {
    logits[i] = (outputTensor->data.int8[i] - outputTensor->params.zero_point)
