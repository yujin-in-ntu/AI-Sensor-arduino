...
운영체제별 설치를 끝낸 뒤 진행하는 **기본 실습**입니다. 학생이 자기 손글씨 숫자
`0~3`을 각 20장씩 직접 촬영하고, 그 데이터로 학습한 모델을 보드에서 추론합니다.

```text
기본 경로: 직접 촬영 → data/camera_digits → 학습 → 03 업로드 → 추론
대체 경로: 촬영 실패 → GitHub 예제 데이터 → 학습 → 03 업로드 → 추론
```

카메라 수집이 정상이라면 공개 예제 데이터로 건너뛰지 말고 1단계부터 진행합니다.
공개 예제는 카메라·포트 문제 또는 시간 부족으로 수집하지 못한 학생을 위한
대체 자료입니다.
...
영상을 갱신하고 빨간 중앙 영역과 명암 차이를 확인합니다. 확인 후 창을 닫아
포트를 해제하고 다음 단계로 이동합니다.
...
카메라 수집을 완료한 학생은 공개 예제가 아니라 자신이 만든
`data/camera_digits`를 사용합니다.
